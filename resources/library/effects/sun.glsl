#property description a star
#property frequency 1

// Panteleymonov A K 2015
// Adapted from https://www.shadertoy.com/view/4lfSzS
...
void main( )
{
    ...
    float time=iTime*iFrequency;

	float mx = time*0.025;
    float my = -0.6;
    vec2 rotate = vec2(mx,my);
	float audioLowPlus = (0.75 + iAudioLow * 0.25);
    ...
    fragColor += vec4( mix(mix(vec3(audioLowPlus,0.0,0.0),vec3(audioLowPlus,0.0,iAudioHi),pow(s2,2.0)),vec3(1.0),pow(s2,10.0))*s2, 1.0 );
    ...
    fragColor.xyz += mix(vec3(1.0,0.6,0.1),vec3(1.0,0.95,1.0),pow(s3,3.0))*s3*audioLowPlus;
	...
	fragColor = composite(texture(iInput, uv), fragColor * iIntensity);
}
